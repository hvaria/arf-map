/**
 * Facility Operations Dashboard — daily command center.
 *
 * Composition:
 *   • Time-aware greeting + at-a-glance status (Header)
 *   • Smart KPI tiles with contextual subtitles (Zone A)
 *   • Alerts & Exceptions panel — urgency-ranked, cross-module list (Zone B)
 *   • Personal Work Queue — items the current user owns (Zone F)
 *   • Today's schedule (collapsible to full OpsCalendar) (Zone D)
 *   • Sticky quick-action bar (Zone G)
 *   • Keyboard shortcuts (g+m, g+i, g+n, g+r, g+c, c, ?)
 *
 * Each zone fails independently so a single broken endpoint cannot blank the
 * whole page. Loading uses skeletons in-place; empty states say "all caught
 * up" plus a forward-looking hint, never a gray box.
 *
 * Team Notes is intentionally NOT a section here — it lives as the bell icon
 * in the header (NotesNotificationButton) so the dashboard above the fold is
 * KPIs and operational widgets only.
 */
...
const QUICK_ACTIONS: Record<
  import("@/lib/roleLens").QuickActionKey,
  { label: string; icon: React.ElementType; href: string }
> = {
  chartMed:        { label: "Chart medication", icon: Pill,           href: "/portal/emar" },
  addIncident:     { label: "Add incident",     icon: AlertTriangle,  href: "/portal/incidents" },
  postNote:        { label: "Post note",        icon: MessageSquare,  href: "/portal/notes" },
  addLead:         { label: "Add lead",         icon: UserPlus,       href: "/portal/crm" },
  openCompliance:  { label: "Compliance",       icon: ShieldCheck,    href: "/portal/compliance" },
};
...
export default function PortalDashboard() {
  ...
  // ── Personal Work Queue ────────────────────────────────────────────────────
  // Derived purely from existing endpoints — no new tables. Once per-staff
  // auth lands, swap text matches for id matches.
        ...
        <section aria-label="Key indicators">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {dashLoading
              ...
              : orderedKpis.map((t) => (
                  <KpiCard key={t.label} tile={t} onClick={() => navigate(t.href)} />
                ))}
          </div>
        </section>
          ...
          <Card>
            <CardContent className="p-0">
              ...
              {alertsLoading ? (
                ...
                <ul>
                  {visibleAlerts.map((a) => (
                    <AlertRow key={a.id} alert={a} onAct={navigate} />
                  ))}
                </ul>
              )}
            </CardContent>
          ...
          <section aria-label="Operations calendar">
            <OpsCalendar
              ...
              onNavigate={(subView, date) => {
                ...
                const dest = map[subView];
                if (dest) navigate(date ? `${dest}?date=${date}` : dest);
              }}
            />
          </section>
        )}
